...
describe('js2d', () => {
  describe('#performance(make2d(array))', ()=> {
    it('performance should be good', ()=> {
      const loops = 20;
      var totalTime = 0
      const maxTime = 10.0 /*ms*/
      ...
      var avgTime = 1.0 * totalTime / loops
      assert(avgTime < maxTime)
    })
  }),
  describe('#make2d(array)', ()=> {
    it('columns as array should return an array of arrays with expected values', ()=> {
      var result = js2d.make2d(columns, obj)
      assert.deepEqual(result.slice(0,4), firstFourRows)
    })
  }),
  describe('#make2d(string)', ()=> {
    it('columns as string, should return an array of arrays with expected values', ()=> {
